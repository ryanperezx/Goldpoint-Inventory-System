...
namespace Goldpoint_Inventory_System.Log
{
    ...
    public partial class TransactLog : UserControl
    {
        ...
        private void LblSearchTransact_MouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            ...
            conn.Open();
            using (SqlCommand cmd = new SqlCommand("SELECT date, [transaction], remarks from TransactionLogs WHERE CAST(date AS date) between @dateFrom and @dateTo ORDER BY CAST(date AS date)", conn))
            {
        ...
        private void fillUpTransactLogs()
        {
            SqlConnection conn = DBUtils.GetDBConnection();
            conn.Open();
            using (SqlCommand cmd = new SqlCommand("SELECT date, [transaction], remarks from TransactionLogs ORDER BY CAST(date AS date)", conn))
            {
                cmd.Parameters.AddWithValue("@dateFrom", txtDateFrom.Text);
                cmd.Parameters.AddWithValue("@dateTo", txtDateTo.Text);
                using (SqlDataReader reader = cmd.ExecuteReader())
